...
object FacebookEventUtils {
    ...
    fun logPaidAdImpressionValue(context: Context?, bundle: Bundle?, mediationProvider: Int) {
        if (mediationProvider == AperoAdConfig.PROVIDER_MAX) AppEventsLogger.newLogger(context!!)
            .logEvent("max_paid_ad_impression_value", bundle) else AppEventsLogger.newLogger(
            context!!
